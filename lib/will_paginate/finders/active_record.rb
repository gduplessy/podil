...
module WillPaginate::Finders
  ...
  module ActiveRecord
    ...
    def paginate_by_sql(sql, options)
      WillPaginate::Collection.create(*wp_parse_options(options)) do |pager|
        ...
        unless pager.total_entries
          ...
          unless %w(oracle oci).include?(connection.adapter_name.downcase)
    ...
    def wp_query(options, pager, args, &block) #:nodoc:
      ...
      if finder == 'find'
        if Array === args.first && !pager.total_entries
    ...
    def wp_count(options, args, finder) #:nodoc:
      ...
      klass = (@owner && @reflection) ? @reflection.klass : self
      count_options = wp_parse_count_options(options, klass)

      # we may have to scope ...
      counter = proc { count(count_options) }

      count = if finder.index('find_') == 0 && klass.respond_to?(scoper = finder.sub('find', 'with'))
                # scope_out adds a 'with_finder' method which acts like with_scope, if it's present
                # then execute the count with the scoping provided by the with_finder
                send(scoper, &counter)
              elsif finder =~ /^find_(all_by|by)_([_a-zA-Z]\w*)$/
                # extract conditions from calls like "paginate_by_foo_and_bar"
                attribute_names = Regexp.last_match(2).split('_and_')
    ...
    def wp_parse_count_options(options, klass) #:nodoc:
      ...
      if options[:select] && options[:select] =~ /^\s*DISTINCT\b/i
      ...
      if count_options[:include] && count_options[:conditions].blank? && count_options[:group].blank?
