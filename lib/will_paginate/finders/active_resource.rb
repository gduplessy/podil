...
module WillPaginate::Finders
  ...
  module ActiveResource
    ...
    def wp_query(options, pager, args, &block) #:nodoc:
      unless args.empty? || args.first == :all
        fail ArgumentError, "finder arguments other than :all are not supported for pagination (#{args.inspect} given)"
      end
    ...
    def instantiate_collection_with_collection(collection, prefix_options = {}) #:nodoc:
      if collection.is_a?(Hash) && collection['type'] == 'collection'
        ...
        collectables = [collectables].compact unless collectables.is_a?(Array)
