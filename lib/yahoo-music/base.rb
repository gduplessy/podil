    ...
    class Base
      class << self
        ...
        def attribute(*args)
          ...
          if Yahoo::Music::Base.subclasses.include?(type.inspect)
            ...
            define_method("#{name}".pluralize.to_sym) do
              value = instance_variable_get("@#{name}") || query_association_by_id(name, id)
        ...
        def name_with_demodulization
          name_without_demodulization.demodulize
        end

        alias_method_chain :name, :demodulization

        def fetch_and_parse(resource, options = {})
          fail YahooWebServiceError, 'No App ID specified' if connection.nil?
          options = options.update('response' => associations.join(',')) if associations.any?
          Hpricot::XML(connection.get(resource, options))
        end
        ...
        def api_path(service, resource, method, *args)
          ...
          parameters.collect! { |param| CGI.escape(param.to_s).downcase }.join('/')
        end
        ...
        def search(*args)
          options = args.extract_options!
          xml = fetch_and_parse(api_path(name, nil, :search, options[:search_mode] || :all, args.join(',')), options)
          xml.search(name).collect { |elem| new(elem) }
        end
      end

      def initialize(xml)
        fail ArgumentError unless xml.is_a?(Hpricot)
      ...
      def query_association_by_id(association, id)
        ...
        xml = query_by_id(id).search(klass.name)
        xml.collect { |elem| klass.new(elem) }
      end
    end
