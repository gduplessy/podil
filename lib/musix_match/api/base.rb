  ...
  module API
    ...
    class Base
      ...
      def self.get(method, params = {})
        fail APIKeyNotSpecifiedException.new('You must specify the API key. MusixMatch::API::Base.api_key = "YOUR_API_KEY"') if api_key.nil?
        ...
        case  parsed_response['message']['header']['status_code']
          when 401 then fail AuthenticationFailedException.new('Authentication failed, probably because of a bad API key')
          when 402 then fail APILimitReachedException.new('A limit was reached, either you exceeded per hour requests limits or your balance is insufficient')
        end
        parsed_response
      end
